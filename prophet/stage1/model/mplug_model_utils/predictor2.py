...
class TextGenerator(object):
    ...
    def __init__(self,
        ...
        self.beam_size = args.beam_size #args['beam_size']
        self.min_length = args.min_length #args['min_length']
        self.max_length = args.max_length #args['max_length']
    ...
    def translate_batch(self, encoder_inputs, do_sample=False, out_size=1, pad_token_id=None):
        """
        ...
        if do_sample:
            ...
            with torch.no_grad():
                return self._fast_translate_batch(encoder_inputs, self.max_length, min_length=self.min_length, do_sample=do_sample,out_size=out_size, pad_token_id=pad_token_id)
    ...
    def _fast_translate_batch(self,
        ...
        batch_size = src_features.size(0) #(batch_size, 614,768)

        src_features = tile(src_features, beam_size, dim=0)#(batch_size*beam_size, 614,768)
        ...
        results["loss"] = [[] for _ in range(batch_size)]  # noqa: F812
        ...
        for step in range(max_length):
            targets_ids = alive_seq.masked_fill(alive_seq == pad_token_id, -100)
            ...
            temp_loss=dec_feat_seq.loss #loss1
            ...
            if do_sample:
                ...
                curr_scores = curr_scores.reshape(-1, beam_size * vocab_size)#yuan
                topk_scores, topk_ids = curr_scores.topk(beam_size, dim=-1)#yuan 
                topk_log_probs = topk_scores * length_penalty #yuan
            ...
            alive_seq = torch.cat(
                [alive_seq.index_select(0, select_indices),
                 topk_ids.view(-1, 1)], -1)
            is_finished = topk_ids.eq(self.end_token) #oyoy
            ...
            if is_finished.any():
                temp_loss=temp_loss.view(alive_seq.size(0),-1)
                temp_loss=-temp_loss
                temp_loss_sum = temp_loss.sum(1)
                temp_loss_sum = temp_loss_sum.view(-1,beam_size)
                soft_loss_sum=F.softmax(temp_loss_sum, dim=-1)
                #temp_loss=temp_loss.view(batch_size,beam_size,-1)#oy
                ...
                for i in range(is_finished.size(0)):
                    ...
                    if end_condition[i]:
                        ...
                        for each in best_hyp[:beam_size]:
                            #score, pred = each
                            score, pred, loss = each
                            results["scores"][b].append(score)
                            #results["scores"][b].append(math.exp(score))
                            results["predictions"][b].append(pred)
                            results["loss"][b].append(loss)
        ...
        for each in results["loss"]:
            losses.append(each[:out_size])
        
        return pred_ids,scores,losses
